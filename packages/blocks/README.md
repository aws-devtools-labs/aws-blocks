...
If you need SQL, prefer `DistributedDatabase` for DSQL-compatible querying. Use `Database` specifically when you need a full (more expensive) Postgres implementation where the engine itself provides and enforces foreign keys, row level security, or triggers; large transactions (more than 3,000 rows); integration with an existing Postgres database; or SQL currently rejected by the `DistributedDatabase` local validator, including JSONB columns and persistent views. Note it carries an idle cost at minimum 0.5 ACU, or a cold start when scaling from zero, unlike the other two blocks.
