...
// The runtime/mock ComputeProvider below needs the condition-resolved compute
// handle plus core's Scope types. The bare specifier resolves to whichever entry
// the condition selects, so this file never sees CDK.
import type { Scope, ScopeParent } from '@aws-blocks/core';
 ...
 * The dashboard is organized by compute: each renders a health section plus
 * logs (always captured) and traces (when the app contains a Tracer). Pass
 * `metrics` for app-wide custom-metric widgets, and toggle sections with the
 * `logs` / `traces` options.
 *
...
/**
 * Declares computes from what a workload needs — runtime/mock entry.
 *
 * The backend module that declares a compute is imported twice: at CDK synth,
 * where the `cdk` entry provisions real infrastructure, and again inside the
 * deployed runtime (and in local dev). At runtime there is nothing to provision —
 * the compute *is* the environment the handler already runs in — so this returns
 * the inert `LambdaCompute` handle: the import resolves, a reference to the
 * compute still points at something, and no CDK is pulled into the Lambda or the
 * browser.
 *
 * Shares its implementation with the `cdk` entry (./compute-provider.ts); only the
 * declared return type differs. Must stay export-compatible with that entry — a
 * customer's backend module imports this name under both conditions.
 */
export const ComputeProvider = {
	/** See the `cdk` entry for the full contract. Inert at runtime. */
	provide(id: string, requirements?: ComputeRequirements, scope?: ScopeParent): Scope {
		return provideCompute(id, requirements, scope) as Scope;
	},
};
