 ...
 * `defaults.logRetention`). The app-wide default log **level** comes from
 * `defaults.logLevel`; a `Logger`'s `level` / `defaultContext` are **runtime**
 * behavior applied by the logger instance itself (per-instance), not deploy
 * config. So the CDK construct is a no-op placeholder that just lets
