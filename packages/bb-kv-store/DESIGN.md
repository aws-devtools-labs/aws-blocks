...
- Conditional write/delete failures throw with `error.name = 'ConditionalCheckFailedException'`, mapped to an `ApiError` with status **409 (Conflict)** so the JSON-RPC serializer emits code 409, not 500. The mock and AWS runtime produce the same name + status (the AWS path re-maps DynamoDB's raw `ConditionalCheckFailedException` on both `put` and `delete`). `retriable` is scoped to the assertion kind: **true only for optimistic-lock value-equals conflicts (`ifValueEquals`)**, where a re-read and retry can succeed; **false for existence/uniqueness assertions (`ifNotExists`, `ifExists`)**, where a blind identical retry fails identically. DynamoDB collapses every conditional failure under one `ConditionalCheckFailedException` with no sub-reason, so the AWS runtime decides retriability per-operation from the condition the caller set on that `put`/`delete`, matching the mock branch-for-branch. When an existence assertion and a value check are **combined** (`ifNotExists`/`ifExists` **and** `ifValueEquals`), the **existence assertion wins** — the conflict is not retriable. Both runtimes derive `retriable` from a single expression keyed on **value presence** (`ifValueEquals !== undefined`, so an explicit `undefined` counts as absent), so mock and AWS produce identical `retriable` for identical inputs, including combined conditions. Note `retriable` marks the conflict *kind* (optimistic-lock value-equals vs existence assertion), **not** a guarantee that a retry will succeed: an `ifValueEquals` conflict against a **missing** row is still flagged retriable, yet a blind retry fails identically — DynamoDB collapses every conditional failure (missing-vs-stale) into one indistinguishable exception, and the mock matches that for parity.
