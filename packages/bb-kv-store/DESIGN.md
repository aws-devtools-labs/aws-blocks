...
- Conditional write/delete failures throw with `error.name = 'ConditionalCheckFailedException'`, mapped to an `ApiError` with status **409 (Conflict)** (retriable) so the JSON-RPC serializer emits code 409, not 500. The mock and AWS runtime produce the same name + status (the AWS path re-maps DynamoDB's raw `ConditionalCheckFailedException` on both `put` and `delete`).
