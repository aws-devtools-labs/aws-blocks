...
describe('Dashboard against a real compute (synth)', () => {
	...
	test('renders logs when the Dashboard is constructed BEFORE the Logger (order-independent via finalize)', async () => {
		const stack = await makeStack('DashboardBeforeLogger');
		const compute = stack._defaultCompute as LambdaCompute;

		// Construct the Dashboard first, then attach logging. Because the widget
		// body is deferred to a finalizer (built after the whole app is
		// constructed), the logs section still appears despite the order.
		new Dashboard(stack, 'dashboard', { routePath: false });
		compute.enableLogging();
		finalizeDashboards(stack);

		const body = dashboardBody(stack);
		assert.ok(body.includes('📋 Logs'), 'logs section renders despite Dashboard-before-Logger order');
	});
});
