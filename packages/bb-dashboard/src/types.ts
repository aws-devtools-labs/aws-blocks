 ...
 * (Metrics, Logger, Tracer) satisfy these types via duck typing, while tests
 * can pass minimal mock objects.
 */
 ...
 *
 * @deprecated No longer consumed by {@link DashboardOptions}. The dashboard is
 * organized by compute and renders a logs section automatically when a Logger
 * is attached to a compute — you do not pass a Logger to the Dashboard. Kept
 * only for backward compatibility; will be removed in a future major.
 */
 ...
 *
 * @deprecated No longer consumed by {@link DashboardOptions}. The dashboard
 * renders a traces section automatically when a Tracer is attached to a
 * compute — you do not pass a Tracer to the Dashboard. Kept only for backward
 ...
 *
 * The dashboard renders your app's compute with a health section plus logs /
 * traces sections automatically when a Logger / Tracer is attached to it (the
 * compute self-reports this). You therefore do **not** pass Logger / Tracer
 * instances here — attaching them is the signal.
 *
