...
export class Database extends Scope {
	...
	constructor(scope: ScopeParent, id: string, options?: DatabaseOptions) {
		...
		if (options?.connection && isConnectionString(options.connection)) {
			...
			options?.connection &&
			'connectionString' in options.connection &&
			typeof options.connection.connectionString !== 'string'
		) {
			...
			const initPromise = connectionString.get().then((connStr) => {
				this.base = new RLSEnabledDatabase(
					new PgClientEngine({
...
function isConnectionString(ref: ExternalDatabaseRef): ref is { connectionString: string; ssl?: ExternalSslOptions } {
