...
import type { SqlQuery, Transaction } from '@aws-blocks/data-common';
import { ENV_NAME_SANITIZE_PATTERN, ENV_VAR_PREFIX } from './constants.js';
import { createCrudHandlers } from './crud/index.js';
import type { CrudMethods, CrudOptions, TableSchema, TableTypeMeta } from './crud/types.js';
...
export class Database extends Scope {
	...
	private async _initBase(): Promise<RLSEnabledDatabase> {
		...
		if (conn && 'connectionString' in conn) {
			const connStr =
				typeof conn.connectionString === 'string' ? conn.connectionString : await conn.connectionString.get();
	...
	private createDataApiEngine(
		...
		const database =
			...
			process.env[`${ENV_VAR_PREFIX}_${envName}_DATABASE`] ||
			envName;
 ...
 */
function createAsyncCrudProxy(
	resolveBase: () => Promise<RLSEnabledDatabase>,
	schema: TableSchema,
	options: CrudOptions<any>,
): any {
