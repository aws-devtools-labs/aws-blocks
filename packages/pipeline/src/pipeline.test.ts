				...
				new Pipeline(
					...
					defaultPipelineProps({
						source: {
							repo: MOCK_REPO,
							connectionArn:
								'arn:aws-us-gov:codeconnections:us-gov-west-1:123456789012:connection/abc-def-123',
						},
					}),
				),
		);
	...
	describe('buildSecrets', () => {
		...
		function synthProjectEnvVars(stack: Stack): Array<{ Name: string; Type: string; Value: unknown }> {
			...
			for (const project of Object.values(projects)) {
				const vars = (project as { Properties?: { Environment?: { EnvironmentVariables?: unknown } } })
