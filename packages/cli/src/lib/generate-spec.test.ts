...
void __dirname; // retained for parity with the source layout; not used after fixture-path fix
// Resolve the real built `api.js` from `@aws-blocks/core`'s dist so the marker
// symbol matches what `generateSpec` discovers. The CLI package has no `api.js`
// of its own, so we locate core's entry and take `api.js` from the same dir.
const require = createRequire(import.meta.url);
const coreDistDir = dirname(require.resolve('@aws-blocks/core'));
const builtApiUrl = pathToFileURL(join(coreDistDir, 'api.js')).href;
