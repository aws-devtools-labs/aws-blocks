/**
 * The layer-render contract — the attach-point seam for nesting front-door layers.
 *
 ...
 * a single layer renders byte-identically to the current `FrontDoorAdapter.render`;
 * the graph renderer consumes `originHandle` to stack layers.
 *
...
/**
 * How a PARENT layer attaches to this layer as an origin. A hostname + protocol
 * covers the common HTTP(S)-origin attach (an ALB DNS name, an API Gateway host,
 * a CloudFront domain, an S3-website endpoint); it can be extended (e.g. with a
 * VPC-origin target for a private ALB) as needed.
 */
 ...
 * `FrontDoorAdapter.render`; nesting is the graph renderer's job.
 */
