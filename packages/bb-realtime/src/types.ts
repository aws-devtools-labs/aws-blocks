...
export interface SubscribeOptions<T = unknown> {
	...
	/** Called after the transport transparently reconnects and at least one channel on the connection has been re-confirmed by the server (with its stored token replayed). Fires once per successful reconnect, after the corresponding `onDisconnect` for the drop that triggered it. Not called on the initial subscribe. If EVERY channel's resubscribe is rejected (e.g. all replayed tokens expired), `onReconnect` does NOT fire — each channel surfaces `onDisconnect('error')` instead. Applies to client-side (hydrated) subscriptions only — a server-side `subscribe()` has no transport reconnect, so an `onReconnect` passed to a server-obtained handle is never fired. */
	onReconnect?: () => void;
}
