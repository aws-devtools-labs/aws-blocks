...
/**
 * CloudFront construct id of the managed API front door within its owner. A stable
...
/**
 * One-shot aspect that provisions the managed API front door at synth — after the
 * whole tree exists.
 *
 ...
 *   and emit a `ApiFrontDoorUrl` output so the distribution can be deployed and
...
class ApiFrontDoorAspect implements IAspect {
	...
	visit(_node: IConstruct): void {
		...
		new cdk.CfnOutput(this.scope, 'ApiFrontDoorUrl', {
			...
			description: 'Blocks API front door URL (CloudFront)',
		});
	}
}
 ...
 * the whole app exists. The API front door is the stable public origin for the
 ...
 */
export function scheduleApiFrontDoor(scope: Construct, apiUrl: string, provision: boolean): void {
