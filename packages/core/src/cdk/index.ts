...
export class BlocksStack extends cdk.Stack implements BaseBlocksStack {
	...
	/** Path to the app's backend module (the `backendCDKPath` / runtime `index.ts`).
	 * Exposed so runtime-hosting constructs (e.g. the Agent BB's AgentCore Runtime) can
	 * co-bundle it into their own deployable artifact. */
