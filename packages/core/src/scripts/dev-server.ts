...
function handleApiRequest(
  ...
  // The whole `/aws-blocks/api` subtree is RPC — a namespaced call arrives at
  // `/aws-blocks/api/{ns}` but is still dispatched by the JSON-RPC body's
  // namespace, so accept the subtree here (matching production/CloudFront).
