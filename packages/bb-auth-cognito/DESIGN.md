...
- **Session freshness.** `revokeUserSessions` revokes Cognito refresh tokens (AWS: `AdminUserGlobalSignOut`; mock: deletes session records). On AWS an already-issued access token stays valid until it expires, so `checkAuth` does not flip immediately — a known mock-vs-AWS parity difference. **Group changes are the exception:** `requireRole` reads membership live (see below), so a grant or revocation applies to a live session on its next request, without a re-login. The cached `cognito:groups` token claim that `requireAuth` / `getCurrentUser` surface still lags until the next sign-in / `fetchAuthSession({ forceRefresh: true })`.
...
3. **`requireRole` reads group membership live; cheaper identity reads use the cached claim.** Authorization is the one place staleness is unacceptable — an admin who runs `addUserToGroup` (or, for revocation, `removeUserFromGroup`) expects it to take effect now, not after the target's token happens to refresh. So `requireRole` reads membership live at call time (AWS: `AdminListGroupsForUser`, paginated; mock: in-process `state.groups`) and returns a `CognitoUser` whose `groups` reflects that read — mirroring how `fetchUserAttributes` reads live rather than trusting the token. The trade-off is one extra Cognito call per guarded request — deduped per request via a `BlocksContext`-keyed memo, so guarding several times in one handler costs one `AdminListGroupsForUser` (Cognito's admin-API quota is low and shared, and this now rides the hot auth path, so throttling here would amplify into cross-route auth failures). The cheap identity reads (`requireAuth` / `getCurrentUser`) deliberately keep decoding the cached `cognito:groups` ID-token claim — no extra call — so their `groups` lags a live change until the next sign-in / refresh. Callers that need live groups outside an authz gate use `requireRole` (or read the returned user's `groups`).
