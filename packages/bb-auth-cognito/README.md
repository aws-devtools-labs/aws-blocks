...
> **Session freshness.** `requireRole` reads group membership **live** (AWS: `AdminListGroupsForUser`; mock: in-process state) on every call, so an admin's `addUserToGroup` / `removeUserFromGroup` takes effect — and a revocation locks the user out — on the user's **next request**, with no re-login. The returned `CognitoUser.groups` reflects that live read. Note the cost/scope trade-off: this is one extra Cognito call per guarded request, and it applies to authorization only. The cheaper identity reads (`requireAuth` / `getCurrentUser` / `signIn`) still surface the `cognito:groups` **token claim**, which lags a live change until the next sign-in or `fetchAuthSession({ forceRefresh: true })`.
>
