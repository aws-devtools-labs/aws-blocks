...
3. **Event Source Mapping** — `SqsEventSource(queue, { batchSize, reportBatchItemFailures: true, maxBatchingWindow })` wired to the shared handler. `batchSize` comes from `options.batchSize` (default 10) and `maxBatchingWindow` from `options.maxBatchingWindowSeconds` (default 5 s, range 0–300). Partial-batch reporting is **always on and not configurable**: it is required for any `batchSize > 1`, because without `ReportBatchItemFailures` in the mapping's `FunctionResponseTypes` a single failing record makes SQS treat the entire batch as handled and delete every message in it — silent data loss. The runtime handler always returns `{ batchItemFailures }` to match. The constructor rejects out-of-range options at synth time (`InvalidOptionException`): `batchSize` must be 1–10 with no batching window, or 1–10000 once `maxBatchingWindowSeconds > 0`, which are the limits AWS enforces on an SQS event source.
