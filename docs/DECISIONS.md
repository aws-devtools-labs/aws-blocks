...
The fix adds `readValidation?: 'off' | 'coerce' | 'strict'`. The open question — raised in PR #283 review — was which value should be the **default**: the lossless `'off'` (raw passthrough, opt into coercion) or `'coerce'` (reconcile on read, opt out for raw). This decision records that choice; the per-BB mechanics live in `packages/bb-distributed-table/DESIGN.md` (D-DT-9).
...
- **A `validateOnRead: boolean` (the PR's first iteration).** Cannot express three distinct behaviors; replaced by the enum before merge.

### `'coerce'` is lossless — it preserves unknown stored keys

An earlier revision of this decision returned the bare validator output on read, which — because most object schemas discard unrecognized keys (Zod `.strip()` by default) — dropped attributes a stored row carried beyond the current schema, and a read-modify-write then persisted the loss (silent data deletion). Raised in PR #283 review (@sarayev). Resolved: `'coerce'` no longer returns the bare validator output; it **deep-merges the coerced value over the raw stored item** so schema output wins per key while undeclared keys (older-schema fields, columns another writer owns), including nested ones, are preserved. Arrays are replaced wholesale (coerced wins). This closes #1007 (reads conform to `T`) *and* preserves data, so the two are no longer in tension. Implemented with `defu` (`createDefu`) rather than a hand-rolled merge — array-as-leaf handling requires custom config in every merge library anyway, and defu is maintained, prototype-safe, zero-dependency, and ESM. The one residual: a schema whose *transform* intentionally removes a key would have it resurrected by the merge; transform-heavy schemas should use `'strict'` or `'off'`. Coercion remains best-effort/validator-dependent for *type* transforms: check-only Standard Schema validators (some Valibot/ArkType schemas) don't fill defaults.
