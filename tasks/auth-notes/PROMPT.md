...
3. Clicking save stores the note for the current user. Store it per-user in a key/value block under the key `note:{username}`.
4. **Persistence:** after a full page reload the visitor is still signed in and their saved note is still shown.
...
After a save, `[data-testid=note-display]` must show exactly the text that was saved.
