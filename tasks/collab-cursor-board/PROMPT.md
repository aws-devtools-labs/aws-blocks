...
2. Submitting the name registers the visitor on the shared presence board and adds a row for them.
...
5. **Persistence:** the board is stored so that after a full page reload the present visitors are still shown.

A single shared board across all tabs — no login, no per-user filtering. Names are visitor-supplied; treat them as untrusted text (don't inject them as markup).
