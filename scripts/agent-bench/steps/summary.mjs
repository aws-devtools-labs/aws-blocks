...
import { buildAggregate, diffAgainstBaseline, renderOverview } from './lib/overview.mjs';
...
// Build this run's compact aggregate (per-cell composites + mean) from the same
// scoring source of truth, then diff it against the baseline aggregate the job
// fetched from S3 for the PR's base commit. The aggregate is also written to
// AGGREGATE_PATH so the job can persist it under bench/runs/<sha>/results.json
// as the baseline a future PR (or this commit, once merged to main) will diff
// against.
...
// Vestigial: the PR-comment step that consumed this file is intentionally
// disabled (summary-only — see agent-bench.yml), so nothing reads SUMMARY_MD_PATH
// today. Kept as a no-op write (guarded on ≥1 cell) only so the commented-out
// comment step stays valid if it is ever re-enabled.
