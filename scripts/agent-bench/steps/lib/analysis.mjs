...
// ── Failure roll-up rendering (pure) ──────────────────────────────────────────
// These fold the structured per-cell failure analyses into markdown for the summary report. Pure and
// deterministic (no SDK / IO), colocated here next to the other pure helpers so they are unit-tested.

// Badge per failure category (falls back to a generic marker for unknown categories).
export const FAILURE_CATEGORY_BADGE = {
	build: '🏗️',
	'dev-server': '🖥️',
	'api-shape': '🔌',
	auth: '🔑',
...
export function fdisp(v, fallback = '—') {
...
export function summarizeFailures(failureRows) {
...
export function renderFailureAnalyses(failureRows) {
	const out = [];
	for (const r of failureRows) {
