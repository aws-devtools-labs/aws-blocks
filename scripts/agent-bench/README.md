...
the matrix runs only on the same repo (no fork PRs) and only when a repo member
applies the `ready-for-pr-benchmark` label (or a member runs `workflow_dispatch`)
— the same-repo + label gate enforced in
...
the role at all. The task `PROMPT.md` files are themselves in-repo and
member-reviewed.
...
These 9 cells cover all 17 Building Blocks across 7 templates. The matrix in
...
**Judge dimensions.** Four shared dimensions (`functional_completeness`,
`selector_contract`, `persistence`, `code_quality`) plus one task-specific
dimension from `tasks/<task>/rubric.md`, all 0–10, **averaged equally** (no
weights — they invite anchoring bias). The overall is recomputed
...
scored cells falls below it. (There is no baseline-delta gate; no baseline
...
| `steps/summary.mjs` | Render the scoreboard (+ collapsible raw per-dimension table, run-logs deep-link) to `$GITHUB_STEP_SUMMARY`; reads one `result.json` per cell (N=1); persist the markdown for the per-run PR comment **only when ≥1 cell produced a result**; optional `BENCH_MIN_SCORE` gate |
...
The summary is also posted as a **new PR comment on every run**, so the bench
history accumulates chronologically (the body's timestamp + run link tell the
comments apart). The hidden `<!-- agent-bench-summary -->` marker is kept in the
body only as a grep aid so a human can spot bench comments — it is **not** used
to find-and-edit a prior comment. When the bench matrix is skipped — the
`ready-for-pr-benchmark` label is absent — no cells run and no artifacts exist,
so `summary.mjs` writes no comment markdown and the post step has nothing to
post (any earlier bench comments stay intact).
