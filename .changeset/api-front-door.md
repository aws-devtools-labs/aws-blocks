...
- The front-door aspect gains a **Hosting-reuse** branch: when a `Hosting` construct is present it already fronts `/aws-blocks/api/*` on its own CloudFront distribution, so the managed API front door reuses it and creates no second distribution. Hosting publishes its distribution via the new `registerHostingDistribution`, and its `addApiBehaviors` reuses `httpOriginFromApiUrl` so both API front-door paths behave identically.
