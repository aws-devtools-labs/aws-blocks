...
Every default is overridable per table via new options — `pointInTimeRecovery`, `deletionProtection`, `encryption` (`'aws-managed' | 'customer-managed'`, or `DistributedTable.fromKmsKey(arn)` to share an existing customer-managed key across tables instead of provisioning one CMK each), and `removalPolicy` (`'retain' | 'destroy'`). Explicit values always win over the environment default. Tables bound via `fromExisting()` are unaffected, and now emit a synth-time warning if durability/encryption options are passed alongside them (they're ignored). An unrecognized `removalPolicy`/`encryption` string also warns at synth rather than silently falling back.
