...
**bb-agent — browser-direct WebSocket streaming (the client-facing win):** on AWS the browser now streams DIRECTLY from the AgentCore Runtime over a WebSocket, bypassing Lambda — so a turn is no longer bounded by the API-Gateway ~30s cap and long-running / streaming agents work for real. `getStreamEndpoint()` returns `{ runtimeArn, wsUrl, sessionId }`; a new client transport `createAgentCoreWsTransport()` (exported from `./client`) opens the socket (JWT passed via the `Sec-WebSocket-Protocol` subprotocol, since browsers can't set WS headers) and feeds `useChat`'s existing `streamChunks` seam. `agentcore-entry.ts` serves a `/ws` handler alongside `/invocations`, driving the same `streamSSE()` loop. Verified end-to-end on AWS, including a >35s turn streaming to completion over the socket.

**bb-agent — server-verified identity on JWT runtimes:** when the Agent has a JWT authorizer (via `auth`), the CDK sets `requestHeaderConfiguration: { allowlistedHeaders: ['Authorization'] }` so AgentCore forwards the gateway-validated caller token to the container (works on both `/invocations` and the browser `/ws` subprotocol path — verified live). The handlers derive `userId` from the token's `sub` claim (`userIdFromContext`) and fall back to a client-supplied `userId` only when no token is forwarded (IAM runtimes). This gives cryptographic proof of identity rather than trusting a client value. Note: for Cognito the derived id is the token `sub` (pool UUID), whereas `CognitoUser.userId` is the username — consistent within the WS path, but relevant when migrating conversations keyed by username. (OIDC token-forwarding parity is a follow-up.)
